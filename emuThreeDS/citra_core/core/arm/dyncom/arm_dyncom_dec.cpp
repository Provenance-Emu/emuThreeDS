...
int BatchDecodeARMInstructions(const u32* instrs, int* indices, int count) {
    ...
    // Process 4 instructions at a time using NEON SIMD
    for (int batch_start = 0; batch_start < count; batch_start += 4) {
        // Determine how many instructions we can process in this batch
        int batch_size = std::min(4, count - batch_start);
        
        // Load instructions into NEON registers
        uint32x4_t instr_vec;
        if (batch_size == 4) {
            // Full batch - load all 4 instructions
            instr_vec = vld1q_u32(instrs + batch_start);
        } else {
            // Partial batch - load only valid instructions
            uint32_t temp[4] = {0};
            std::memcpy(temp, instrs + batch_start, batch_size * sizeof(uint32_t));
            instr_vec = vld1q_u32(temp);
        }
        
        // Extract keys for all 4 instructions at once
        // Shift right by 20 bits and mask with 0xFF
        uint32x4_t shifted = vshrq_n_u32(instr_vec, 20);
        uint32x4_t keys = vandq_u32(shifted, vdupq_n_u32(0xFF));
        
        // Convert to array for processing
        uint32_t key_array[4];
        vst1q_u32(key_array, keys);
        
        // Process each instruction in the batch
        for (int i = 0; i < batch_size; i++) {
            u32 instr = instrs[batch_start + i];
            u8 key = static_cast<u8>(key_array[i]);
            ...
            for (int j = 0; j < entry.num_candidates; j++) {
                int candidate_idx = entry.indices[j];
                ...
                if (CheckInstructionMatch(instr, arm_instruction[candidate_idx])) {
                    ...
                    // Found a match!
                    indices[batch_start + i] = candidate_idx;
                    found_match = true;
                    success_count++;
                    break;
                }
            ...
            // If no match found via lookup table, do a full search as fallback
            if (!found_match) {
                int instr_slots = sizeof(arm_instruction) / sizeof(InstructionSetEncodingItem);
                
                for (int j = 0; j < instr_slots; j++) {
                    // Skip VFP3 instructions
                    if (arm_instruction[j].version == ARMVFP3)
                        continue;
                    
                    // Skip instructions we already checked via the lookup table
                    bool already_checked = false;
                    for (int k = 0; k < entry.num_candidates; k++) {
                        if (entry.indices[k] == j) {
                    ...
                    // Check if instruction matches the pattern
                    if (CheckInstructionMatch(instr, arm_instruction[j])) {
                        // Check exclusions
                        if (CheckExclusionMatch(instr, arm_exclusion_code[j])) {
                            continue; // Exclusion matched, so this isn't the right instruction
                        }
                        
                        // Found a match!
                        indices[batch_start + i] = j;
                        found_match = true;
                        success_count++;
                        break;
                    }
            ...
            // If still no match found, mark this instruction as invalid
            if (!found_match) {
                // Use -1 to indicate no match found
                indices[batch_start + i] = -1;
                LOG_ERROR(Core_ARM11, "Failed to decode ARM instruction: 0x%08X", instr);
            }
        }
    }
...
bool BatchCheckInstructionMatch(uint32x4_t instr_vec, const InstructionSetEncodingItem& pattern) {
    ...
    while (n) {
        if (pattern.content[base + 1] == 31 && pattern.content[base] == 0) {
            // Special case for clrex and other full-word matches
            uint32x4_t pattern_vec = vdupq_n_u32(pattern.content[base + 2]);
            uint32x4_t cmp_result = vceqq_u32(instr_vec, pattern_vec);
            
            // If any instruction doesn't match, return false
            ...
            uint32x4_t mask = vdupq_n_u32((1U << num_bits) - 1);
            uint32x4_t expected = vdupq_n_u32(expected_value);
            
            // We need to handle variable shift amounts since NEON requires constant shifts
            // Extract bits from each instruction individually
            uint32_t extracted_vals[4];
            extracted_vals[0] = (vgetq_lane_u32(instr_vec, 0) >> start_bit) & ((1U << num_bits) - 1);
            extracted_vals[1] = (vgetq_lane_u32(instr_vec, 1) >> start_bit) & ((1U << num_bits) - 1);
            extracted_vals[2] = (vgetq_lane_u32(instr_vec, 2) >> start_bit) & ((1U << num_bits) - 1);
            extracted_vals[3] = (vgetq_lane_u32(instr_vec, 3) >> start_bit) & ((1U << num_bits) - 1);
            
            // Load extracted values back into a vector
            uint32x4_t extracted = vld1q_u32(extracted_vals);
            
            // Compare with expected value
            uint32x4_t cmp_result = vceqq_u32(extracted, expected);
